(function (window) {
    ...
    ThreeMaze.prototype.onGenerateMaze = function () {
        var generateResult = this.generateMaze(this.side);
        var new_map = generateResult.map;
        var collectibles = generateResult.collectibles;
        var new_player_path = [];
        var latency = 50;
        var self = this;
        var tween = null;
        for (var x = this.side; x > 0; x -= 1) {
            ...
            for (var y = 1; y < this.side + 1; y += 1) {
                var delay =
                    (this.side - x) * latency + (this.side - y) * latency;
                // Inits player path
                new_player_path[x][y] = false;

                // Removes old mesh if needed
                if (
                    typeof this.map[x] != "undefined" &&
                    typeof this.map[x][y] != "undefined" &&
                    typeof this.map[x][y] === "object"
                ) {
                ...
                if (
                    typeof this.player.path != "undefined" &&
                    typeof this.player.path[x] != "undefined" &&
                    typeof this.player.path[x][y] != "undefined" &&
                    typeof this.player.path[x][y] === "object"
                ) {
                ...
                if (new_map[x][y] === 0) {
                    // Generates the mesh
                    var wall_geometry = new THREE.CubeGeometry(
                        this.thickness,
                        this.thickness,
                        this.thickness,
                        1,
        ...
        var end_hide_tween = new TWEEN.Tween({
            scale: 1,
            y: this.thickness / 2,
            mesh: this.end,
        }).to({ scale: 0, y: 0 }, 300);
        var end_show_tween = new TWEEN.Tween({ scale: 0, y: 0, mesh: this.end })
            .to(
                {
                    scale: 1,
                    y: this.thickness / 2,
    ...
    ThreeMaze.prototype.initScene = function () {
        ...
        var head_mesh = new THREE.Mesh(
            new THREE.SphereGeometry(this.thickness / 2, 9, 9),
            this.materials.red
        );
        var body_mesh = new THREE.Mesh(
        ...
        this.end.position.set(
            -((this.side / 2) * this.thickness) + this.thickness * 2,
            0,
            -((this.side / 2) * this.thickness) + this.thickness * 2
        );
        this.end.scale.y = 0;
        ...
        geometry.vertices.push(
            ...
            new THREE.Vector3(Math.sqrt(3) * (this.side * this.thickness)),
            0,
            0
        );
    ...
    ThreeMaze.prototype.onKeyDown = function (evt) {
        ...
        // Check if object in player's path is a collectible
    ...
    ThreeMaze.prototype.movePlayer = function (animate) {
        ...
        var to = {
            height: Math.PI,
            x:
                -((this.side * this.thickness) / 2) +
                this.player.mazePosition.x * this.thickness,
            z:
                -((this.side * this.thickness) / 2) +
                this.player.mazePosition.z * this.thickness,
        };
        ...
        tween.onUpdate(function () {
            ...
            this.mesh.position.y =
                (Math.cos(this.height) + 1) * (self.thickness / 4);
            this.mesh.position.z = this.z;
        });
        // End of the maze: starts again
        tween.onComplete(function () {
            if (
                self.player.mazePosition.x === 2 &&
                self.player.mazePosition.z === 2
            ) {
                self.onGenerateMaze();
            }
        });
    ...
    ThreeMaze.prototype.onMouseMove = function (evt) {
        if (this.camera.clicked !== false) {
            var target_rotation = {
                z:
                    this.cameraHelper.rotation.z +
                    (evt.pageY - this.camera.clicked.y) / 800,
                y:
                    this.cameraHelper.rotation.y +
                    (this.camera.clicked.x - evt.pageX) / 800,
            };
    ...
    ThreeMaze.prototype.render = function () {
        ...
        if (this.cameraHelper.targetRotation !== false) {
            this.cameraHelper.rotation.z +=
                (this.cameraHelper.targetRotation.z -
                    this.cameraHelper.rotation.z) /
                10;
            this.cameraHelper.rotation.y +=
                (this.cameraHelper.targetRotation.y -
                    this.cameraHelper.rotation.y) /
                10;
        }
        this.camera.position = this.cameraHelper.geometry.vertices[1]
            .clone()
            .applyProjection(this.cameraHelper.matrixWorld);
