...
ThreeMaze.prototype.generateMaze = function (size) {
    ...
    // Collectibles
    let collectiblesToBePlaced = 5; // Controls how many collectibles are added
    let collectibles = [];

    // Find valid spots for the collectibles
    while (collectiblesToBePlaced > 0) {
        // Pick random spot in the map
        let row = Math.floor(Math.random() * 30 + 1);
        let col = Math.floor(Math.random() * 30 + 1);

        // If it is available, place a collectible there
        if (map[row][col] != null && map[row][col] === 1) {
            collectibles.push({ x: row, y: col });

            // Reduce remaining collectible count
            collectiblesToBePlaced--;
        }
    }

    return { map: map, collectibles: collectibles };
};
